...
import { PaperDetails } from "../../_components";
import "./style.scss";
...
export async function generateMetadata({ params }) {
  ...
  return generate({
    title: isNew ? "Create New Paper" : "Edit Paper",
    description: `${isNew ? "Create" : "Edit"} a Paper`,
  });
}

export default function PaperId({ params }) {
  if (!params.id) {
    return notFound();
  }
      ...
      <div className="row mb-20">
        <div className="col-12">
          <Title className={"d-flex"}>
            <BackButton className="mr-8" href={routes.PAPERS} />
            {isNew ? "New paper" : "Edit paper"}
          </Title>
        </div>
      </div>
      <PaperDetails data={data} isNew={isNew} />
      {!isNew && (

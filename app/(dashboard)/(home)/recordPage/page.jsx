...
const initialPatientData = {
    ...
    phone: "+55 (16) 99123-4567",
    address: "Av. Bandeirantes, 3900, Ribeirão Preto, SP",
    memberSince: "2022-03-05",
    insurance: {
        provider: "DentalCorp Plus",
        policyNumber: "DC987654321"
    },
    medicalAlerts: "Alergia a Penicilina. Histórico de hipertensão.",
    lastVisit: "2024-01-20",
    nextAppointment: "2025-07-25",
    chiefComplaint: "Sensibilidade no molar superior direito.",
    clinicalNotes: "Paciente relata dor aguda ao consumir bebidas geladas. Exame radiográfico do dente 3 revela necessidade de tratamento de canal. Foi realizado procedimento de profilaxia e agendado o tratamento endodôntico. Oclusão estável, sem sinais de bruxismo."
};
...
export default function PatientRecordPage() {
    ...
    const TextAreaField = ({ label, value, name, ...props }) => (
        isEditing ? (
            <FormField id={name} label={label} value={value} onChange={handleInputChange} component="textarea" {...props} />
        ) : (
            <div className="infoField">
                <span className="infoLabel">{label}</span>
                <p className="infoValue" style={{ whiteSpace: 'pre-wrap' }}>{value || "N/A"}</p>
            </div>
        )
    );
        ...
        <div className="pageContainer">
            ...
            <form onSubmit={handleSave} className="formContainer">
                <section className="patientHeader">
                    ...
                    <div className="patientInfo">
                        ...
                        <p className="patientMeta">
                            Paciente desde {new Date(patientData.memberSince).toLocaleDateString()}
                ...
                {patientData.medicalAlerts && (
                    <section className="medicalAlert">
                        <strong>Alertas Médicos:</strong> {patientData.medicalAlerts}
                    </section>
                )}

                <div className="contentGrid">
                    <section className="formSection">
                        <h3 className="sectionTitle">Contato & Convênio</h3>
                        <div className="fieldsWrapper">
                             <InfoField label="Data de Nascimento" name="dob" value={patientData.dob} type="date"/>
                             <InfoField label="Endereço de Email" name="email" value={patientData.email} type="email"/>
                             <InfoField label="Telefone" name="phone" value={patientData.phone} type="tel"/>
                             <InfoField label="Endereço Completo" name="address" value={patientData.address} />
                             <InfoField label="Convênio" name="insurance.provider" value={patientData.insurance.provider} />
                             <InfoField label="Nº da Apólice" name="insurance.policyNumber" value={patientData.insurance.policyNumber} />
                        </div>
                    </section>
